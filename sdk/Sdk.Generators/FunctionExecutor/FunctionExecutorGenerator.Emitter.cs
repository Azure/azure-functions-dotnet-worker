...
namespace Microsoft.Azure.Functions.Worker.Sdk.Generators
{
    public partial class FunctionExecutorGenerator
    {
        internal static class Emitter
        {
            ...
            {
                var anyDefaultExecutor = functions.Any(f => f.Visibility == FunctionMethodVisibility.PublicButContainingTypeNotVisible);
                ...
                string result = $$"""
                         ...
                         namespace {{FunctionsUtil.GetNamespaceForGeneratedCode(context)}}
                         {
                             ...
                             {
                                 private readonly IFunctionActivator _functionActivator;{{(anyDefaultExecutor ? $"{Environment.NewLine}        private Lazy<IFunctionExecutor> _defaultExecutor;" : string.Empty)}}
                                 ...
                                 {
                                     {{GetMethodBody(functions, context, anyDefaultExecutor)}}
                                 }{{(anyDefaultExecutor ? $"{Environment.NewLine}{GetLazyDefaultExecutorMethod(context)}" : "")}}
                             }
            ...
            private static string GetLazyDefaultExecutorMethod(GeneratorExecutionContext context)
            {
                var coreAssembly = context.Compilation.SourceModule.ReferencedAssemblySymbols.Where(a => a.Name == "Microsoft.Azure.Functions.Worker.Core").Single();
                var assemblyIdentity = coreAssembly.Identity;
            ...
            private static string GetMethodBody(IEnumerable<ExecutableFunction> functions, GeneratorExecutionContext context, bool anyDefaultExecutor)
            {
                ...
                {{(anyDefaultExecutor ? $"            _defaultExecutor = new Lazy<IFunctionExecutor>(() => GetDefaultExecutor(context));{Environment.NewLine}" : string.Empty)}}
                """);
