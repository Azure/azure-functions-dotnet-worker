...
namespace Microsoft.Extensions.Hosting
{
    ...
    {
        private static readonly object AutoConfigureStartupStateKey = new();
        ...
        public static IHostBuilder InvokeAutoGeneratedConfigureMethods(this IHostBuilder builder)
            => builder.InvokeAutoGeneratedConfigureMethods(WorkerApplicationAssemblyContext.ResolveOrEntryAssembly());

        /// <summary>
        /// Invokes auto-generated configuration methods from the specified function application assembly.
        /// </summary>
        /// <param name="builder">The <see cref="IHostBuilder"/> to configure.</param>
        /// <param name="applicationAssembly">The function application assembly that owns generated startup.</param>
        /// <returns>The same <see cref="IHostBuilder"/> after the auto-generated configuration methods are invoked.</returns>
        public static IHostBuilder InvokeAutoGeneratedConfigureMethods(this IHostBuilder builder, Assembly applicationAssembly)
        {
            if (builder is null)
            {
            ...
            if (builder.Properties.TryGetValue(AutoConfigureStartupStateKey, out object? value))
            {
                var existing = (AutoConfigureStartupState)value;
                ...
                throw new InvalidOperationException("Auto-generated worker configuration is already running for this host builder.");
            }

            var state = new AutoConfigureStartupState(applicationAssembly);
            builder.Properties[AutoConfigureStartupStateKey] = state;

            try
            {
                var autoConfigureStartupTypes = applicationAssembly
                .GetTypes()
        ...
        private sealed class AutoConfigureStartupState
        {
            internal AutoConfigureStartupState(Assembly applicationAssembly)
            {
            ...
            internal void EnsureCompatible(Assembly applicationAssembly)
            {
                if (ApplicationAssembly != applicationAssembly)
                {
                    throw new InvalidOperationException(
                        $"Auto-generated worker configuration is already associated with '{ApplicationAssembly.FullName}' and cannot also use '{applicationAssembly.FullName}'.");
                }
            }
