...
{
    public class EventGridBinaryDataArrayConverterTests
    {
        private EventGridBinaryDataArrayConverter _eventGridConverter;

        public EventGridBinaryDataArrayConverterTests()
        {
            var host = new HostBuilder().ConfigureFunctionsWorkerDefaults((WorkerOptions options) => { }).Build();
            var logger = host.Services.GetService<ILogger<EventGridBinaryDataArrayConverter>>();

            _eventGridConverter = new EventGridBinaryDataArrayConverter(logger);
        }
