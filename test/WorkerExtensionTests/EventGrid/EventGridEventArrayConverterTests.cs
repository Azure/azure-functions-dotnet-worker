...
{
    public class EventGridEventArrayConverterTests
    {
        private EventGridEventArrayConverter _eventGridConverter;

        public EventGridEventArrayConverterTests()
        {
            var host = new HostBuilder().ConfigureFunctionsWorkerDefaults((WorkerOptions options) => { }).Build();
            var logger = host.Services.GetService<ILogger<EventGridEventArrayConverter>>();

            _eventGridConverter = new EventGridEventArrayConverter(logger);
        }
