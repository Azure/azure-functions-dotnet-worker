...
{
    public class EventGridMultipleCloudEventConverterTests
    {
        private EventGridMultipleCloudEventConverter _eventGridConverter;

        public EventGridMultipleCloudEventConverterTests()
        {
            var host = new HostBuilder().ConfigureFunctionsWorkerDefaults((WorkerOptions options) => { }).Build();
            var logger = host.Services.GetService<ILogger<EventGridMultipleCloudEventConverter>>();

            _eventGridConverter = new EventGridMultipleCloudEventConverter(logger);
        }
